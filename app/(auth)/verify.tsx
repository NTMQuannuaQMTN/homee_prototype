...
export default function Verify() {
    ...
    const { signupInfo, setUser, setSession, password } = useAuthStore();
    ...
    const checkCode = async (inputCode?: string) => {
        ...
        if (codeToCheck.length !== MAXLENGTH || (!signupInfo?.email && !signupInfo?.phone)) {
            setValid(false);
            return;
        }
        ...
        if (signupInfo.email) {
            ...
            // Always use lowercase for email
            const phoneNum = signupInfo.phone.trim().toLowerCase();

            const { data, error } = await supabase.auth.verifyOtp({
                phone: phoneNum,
                token: codeToCheck,
                type: "sms", // Use "email_otp" if "email" doesn't work for your supabase-js version.
            });
